...
async function main() {
  ...
  const targetVideo = "Gallatin Navigator - Tradewinds.mp4";
  ...
  console.log(`Processing single video: ${targetVideo}`);
  
  try {
    const metadata = await extractVideoMetadata(join(DOWNLOADS_DIR, targetVideo));
    
    // Save metadata to file
    const outputPath = join(DOWNLOADS_DIR, `${targetVideo.replace('.mp4', '')}_metadata.json`);
